...
ACC = colors.HexColor("#2563c9")
SUB = colors.HexColor("#54637e")
LINE = colors.HexColor("#ccd6e6")
BOXB = colors.HexColor("#eef3fb")
BOXY = colors.HexColor("#fff5e0")
...
def why(t):
    S.append(Paragraph("<b>Why this matters.</b> " + t, CALL))


def check(t):
    S.append(Paragraph("<b>Check your understanding.</b> " + t, CHECK))


def rule():
    S.append(HRFlowable(width="100%", thickness=0.6, color=LINE, spaceBefore=4, spaceAfter=8))


def fig(path, caption, maxw=470, maxh=560):
    if not os.path.exists(path):
        P("[figure missing: %s]" % os.path.basename(path), CAP)
        return
...
rule()
P(
    "This document explains every stage of the project from first principles: what each "
    ...
    "ones produced by the analysis scripts.",
    BODY,
)
P(
    "<b>Project:</b> CIS*6560 Research Project, MCTI, University of Guelph. "
    "<b>Dataset:</b> UNSW-NB15 (partitioned set). <b>Protocol:</b> 5 random seeds, identical "
    "pipeline across all model families.",
    BODY,
)
gap(6)
...
toc = [
    "1. The big picture",
    "2. Data integrity and ingestion",
    "3. Exploratory data analysis (EDA)",
    "4. Preprocessing pipeline",
    "5. Class-imbalance handling",
    "6. Feature engineering",
    "7. Models and architectures",
    "8. Evaluation protocol and headline results",
    "9. RQ1 &mdash; feature-engineering strategy and feature groups",
    "10. RQ2 &mdash; which classes stay hard, and why",
    "11. Limitations and honest scope",
    "12. Explain this project in three minutes",
    "13. Glossary",
]
for t in toc:
    P(t, TOCI)
...
P(
    "An <b>intrusion detection system (IDS)</b> watches network traffic and flags activity that "
    ...
    "the model, not to inconsistent treatment.",
    BODY,
)
P("Two research questions drive the analysis:", BODY)
bullets(
    [
        "<b>RQ1:</b> Which feature-engineering strategy, and which kinds of features, best "
...
P(
    "The data flows through eight stages, shown in the map below: an integrity check, exploratory "
    ...
    "walks each stage in that order.",
    BODY,
)
fig(
    FIG + "/data_pipeline_mindmap.png",
    "Figure 1. The full data journey, stage by stage, with a one-line rationale per step.",
    maxw=430,
    maxh=600,
)
why(
    "A comparison is only meaningful if it is fair. Fixing one pipeline and varying only the model "
...
check(
    "In your own words, why is it important that all six models share the same train/validation/test "
...
P(
    "<b>What it is.</b> Before any analysis, each downloaded data file is checked against a stored "
    ...
    "match and the pipeline stops.",
    BODY,
)
P(
    "<b>Why it matters.</b> Every downstream number depends on the exact input bytes. A silent "
    "corruption would invalidate the entire study, and you would not know. The checksum makes the "
    "starting point reproducible and verifiable by anyone who re-runs the project.",
    BODY,
)
P(
    "<b>What we load.</b> The project uses the UNSW-NB15 <i>partitioned</i> distribution: a training "
...
check(
    "If two people download the dataset on different days and get the same SHA-256 value, what "
...
P(
    '<b>What it is.</b> EDA is the "get to know your data" phase: nine analyses that profile the '
...
fig(
    ...
    "Figure 2. Class distribution. The dataset is dominated by Normal traffic and a few large attack types; several attack classes are extremely rare.",
    maxw=420,
    maxh=300,
)
P(
    "The single most important finding is <b>severe class imbalance</b>. In the test split, Normal "
    ...
    "training tricks that refuse to let it cheat (Sections 5 and 8).",
    BODY,
)
P(
    "Correlation and skewness analyses reveal two more things that directly drive preprocessing: many "
    ...
    "warning of which classes will be hard.",
    BODY,
)
fig(
    EDA + "/05_pca_projection.png",
    "Figure 3. PCA projection. Linear structure: some classes separate, others overlap, hinting at intrinsic difficulty.",
    maxw=360,
    maxh=280,
)
why(
    "EDA is where you earn the right to make modeling choices. The imbalance finding justifies "
...
check(
    "Why can a classifier reach 90%+ raw accuracy on this dataset while being almost useless at "
    "catching Worms?"
)
S.append(PageBreak())
...
P(
    "Preprocessing turns raw, messy records into clean numerical matrices, while obeying one rule "
    ...
    "scores would be optimistically wrong.",
    BODY,
)
P(
    "<b>Cleaning.</b> We drop the artificial <i>id</i> column (an index, not a signal), remove "
    ...
    "computed on training data only (the median is robust to the heavy skew we found in EDA).",
    BODY,
)
P(
    "<b>Encoding.</b> Three columns are categorical text (protocol, service, connection state). "
    ...
    "the feature count grows to 190.",
    BODY,
)
P(
    "<b>Splitting and scaling.</b> We make a <b>70/15/15 stratified split</b> into train, validation "
    ...
    "stacks consecutive records into short sequences for the LSTM.",
    BODY,
)
why(
    "The dual scaling is not an accident; it is the difference between a working deep model and a "
...
check(
    "We compute the median for imputation on the training fold only, then reuse it on the test "
...
P(
    "Because rare classes are easy to ignore, we counter the imbalance in three complementary ways, "
    "all applied to the <b>training fold only</b> (resampling the test set would fake the results):",
    BODY,
)
bullets(
    [
...
P(
    "Feature engineering asks: do we need all 190 features, and can a cleverly chosen subset do "
    "better? Four strategies are compared, all judged under one fixed Random Forest so the comparison "
    "isolates the <i>features</i>, not the model:",
    BODY,
)
bullets(
    [
...
P(
    "The ablation result is decisive and slightly counter-intuitive: <b>fewer, better-chosen features "
    ...
    "also far cheaper to train.",
    BODY,
)
fig(
    FIG + "/rq1_ablation_strategy.png",
    "Figure 4. Feature-engineering strategy comparison (fixed Random Forest, multiclass). RFA with 14 features wins; piling on features hurts.",
    maxw=430,
    maxh=300,
)
why(
    "This is a real, publishable finding and a classic lesson: dimensionality is not free. The best "
...
P(
    "All six models implement the same interface and are driven by the same harness, so they are "
    "trained and scored identically. Briefly, in increasing complexity:",
    BODY,
)
bullets(
    [
...
P(
    "<b>The deep-learning rescue (a key story).</b> Initially the CNN and LSTM performed terribly &mdash; "
    ...
    "epoch. After these changes the deep models train cleanly and land in a sensible range (Section 8).",
    BODY,
)
why(
    'The lesson is that "deep learning doesn\'t work here" is often really "the inputs were wrong." '
    "Diagnosing the scaling bug, rather than abandoning the models, is what produced honest, "
    "comparable deep-learning numbers."
)
check(
    "Why are tree models unaffected by unscaled billion-valued features while neural networks are "
    "crippled by them?"
)
...
P(
    "Every model is trained with <b>five random seeds</b> (42 to 46) and we report the mean and "
...
P(
    "Two clear stories. On the <b>binary</b> task (attack vs normal) everything works well; XGBoost "
    ...
    "&mdash; gradient-boosted trees are simply strong on this kind of data.",
    BODY,
)
P(
    "The training curves below confirm the deep models are now <b>well-trained, not overfit</b>: "
    "training and validation loss descend together, with validation at or below training (a "
    "consequence of dropout). The generalization-gap chart then makes a subtler point.",
    BODY,
)
fig(
    FIG + "/dl_training_curves.png",
    "Figure 5. Deep-model train vs validation curves (mean +/- std over 5 seeds). Curves track together; no diverging gap = no overfitting.",
    maxw=430,
    maxh=560,
)
fig(
    FIG + "/train_test_gap.png",
    "Figure 6. Train-vs-test macro-F1 gap. Tree ensembles win on test score but overfit multiclass (RF gap 0.31, XGBoost 0.19); the CNN barely overfits at all.",
    maxw=470,
    maxh=320,
)
P(
    "The gap chart reveals an honesty point the leaderboard hides: <b>Random Forest and XGBoost buy "
...
check(
    "XGBoost has the best multiclass test score but a 0.19 train-test gap; the CNN scores lower but "
...
P(
    "RQ1 has two halves. The first &mdash; which <i>strategy</i> wins &mdash; was answered in Section 6: "
    ...
    "drops. A big drop means the class depends on that group.",
    BODY,
)
P(
    "First, the winning RFA selection is dominated by <b>basic flow</b> features &mdash; 10 of its 14 "
    ...
    "of basic volume statistics carry most of the multiclass signal.",
    BODY,
)
fig(
    FIG + "/rq1_rfa_group_composition.png",
    "Figure 7. Semantic groups inside the 14-feature RFA selection. Basic flow statistics dominate.",
    maxw=360,
    maxh=260,
)
fig(
    FIG + "/rq1_feature_group_importance.png",
    "Figure 8. Per-class feature-group importance (permutation: F1 drop when a group is shuffled). Darker = more essential.",
    maxw=420,
    maxh=330,
)
...
why(
    'RQ1 turns "which features matter" from a guess into a measured, per-class answer. The negative-'
    "importance finding is the kind of non-obvious result that makes a comparative study worth "
    "publishing."
)
check(
    "If a feature group has negative permutation importance for a class, what is that telling you "
...
P(
    "RQ2 asks which attack types remain hard <i>regardless of model</i>, and diagnoses the cause. We "
    "average each class's F1 across all six models to get a difficulty ranking, then read the confusion "
    "structure to explain it.",
    BODY,
)
fig(
    FIG + "/rq2_class_difficulty.png",
    "Figure 9. Class difficulty (mean F1 across all six models; n = test support). Difficulty is not simply about rarity.",
    maxw=440,
    maxh=300,
)
fig(
    FIG + "/rq2_per_class_f1_heatmap.png",
    "Figure 10. Per-class F1 for every model. The hard classes (left) stay red across the whole row of models.",
    maxw=470,
    maxh=250,
)
P(
    "The difficulty order, hardest first, is Backdoor (0.13), Analysis (0.15), DoS (0.17), Worms (0.20) "
    ...
    "not a quirk of one model. That is the core RQ2 answer.",
    BODY,
)
...
why(
    'RQ2 converts a vague worry ("some classes are hard") into specific, actionable diagnoses: which '
    "classes, how hard, and the exact confusions responsible. That is what a security team would need "
    "to decide where to invest."
)
check(
    "Shellcode has high recall but low F1. Explain, in terms of false positives, why a high-recall "
...
P(
    "Owning the project means owning its limits. The following are stated plainly and were design "
...
check(
    'A reviewer says "your LSTM result proves recurrent models work for IDS." Why is that claim too '
...
P(
    '"I built a fair comparison of rule-based, classical, and deep-learning intrusion detectors on '
    ...
    'The trees win on score but overfit; the CNN generalizes better than its score suggests."',
    BODY,
)
...
gl = [
    (
        "SHA-256",
        "A cryptographic fingerprint of a file; any change to the bytes changes the fingerprint.",
    ),
    (
        "Leakage",
        "When information from validation/test influences training, inflating scores dishonestly.",
    ),
    ...
    (
        "RFA",
        "Recursive Feature Addition: greedily add the most helpful feature until it stops helping.",
    ),
    (
        "Permutation importance",
        "Shuffle a feature/group and measure the score drop to gauge its value.",
    ),
    ...
    (
        "Overfitting",
        "Learning the training data too literally; shows up as a large train-test gap.",
    ),
...
def footer(c, d):
    ...
    c.drawCentredString(
        letter[0] / 2,
        30,
        "Multi-Model IDS on UNSW-NB15  -  Methodology, Results and Analysis  -  page %d" % d.page,
    )
